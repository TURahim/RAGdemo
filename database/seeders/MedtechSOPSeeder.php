...
class MedtechSOPSeeder extends Seeder
{
    ...
    protected function getDepartmentStructure(): array
    {
        return [
            ...
            [
                ...
                'description' => 'Employee training, onboarding, and personnel qualification procedures.',
                'owner' => 'qa_manager',
                'books' => [
                    [
                        'name' => 'Training Management',
                        'description' => 'Procedures for employee training, qualification, and competency verification.',
                        'chapters' => [
                            [
                                'name' => 'Employee Training',
                                'pages' => [
                                    $this->sopNewHireTraining(),
                                    $this->sopTrainingRecords(),
                                    $this->sopCompetencyAssessment(),
                                ],
                            ],
                            [
                                'name' => 'Role-Specific Training',
                                'pages' => [
                                    $this->sopProductionTraining(),
                                    $this->sopQualityTraining(),
                                ],
                            ],
                        ],
                    ],
    ...
    // ========================================
    // Human Resources / Training SOPs
    // ========================================

    protected function sopNewHireTraining(): array
    {
        return [
            'name' => 'SOP-HR-001: New Employee Training Requirements',
            'approval_state' => 'approved',
            'review_interval_days' => 365,
            'html' => $this->buildSopHtml(
                'SOP-HR-001',
                'New Employee Training Requirements',
                'This procedure defines the training requirements for all new employees joining MedTech Solutions Inc.',
                'Human Resources, Department Managers, Training Coordinators, All New Employees.',
                [
                    'HR initiates new hire onboarding within first day of employment.',
                    'All new employees must complete General Orientation within first week.',
                    'General Orientation includes: Company overview, Quality Policy, Safety procedures, IT systems access.',
                    'Department Manager assigns role-specific training plan within 3 business days.',
                    'New employees must complete QMS Awareness Training (ISO 13485, FDA QSR basics) within 30 days.',
                    'GMP (Good Manufacturing Practice) training required for all production personnel within 30 days.',
                    'Document Control training required before accessing controlled documents.',
                    'Job-specific SOPs must be read and acknowledged before performing related tasks.',
                    'Supervisor verifies competency through observation and/or written assessment.',
                    'All training must be documented in the Training Management System.',
                    'New hire training must be completed within 90 days of start date.',
                    'HR reviews training completion status at 30, 60, and 90 day checkpoints.',
                ],
                ['New Hire Training Checklist (HR-F-001)', 'Training Plan Template (HR-F-002)', 'Competency Verification Form (HR-F-003)']
            ),
        ];
    }

    protected function sopTrainingRecords(): array
    {
        return [
            'name' => 'SOP-HR-002: Training Records Management',
            'approval_state' => 'approved',
            'review_interval_days' => 365,
            'html' => $this->buildSopHtml(
                'SOP-HR-002',
                'Training Records Management',
                'This procedure defines requirements for creating, maintaining, and retaining employee training records.',
                'Human Resources, Training Coordinators, Department Managers.',
                [
                    'All training must be documented within 5 business days of completion.',
                    'Training records include: Employee name, training topic, date, duration, trainer, and assessment results.',
                    'Electronic training records are maintained in the Training Management System (TMS).',
                    'Training certificates and external course completions must be uploaded to TMS.',
                    'Read-and-understand acknowledgments must include employee signature and date.',
                    'Competency assessments must document pass/fail criteria and results.',
                    'Retraining requirements are documented when competency is not demonstrated.',
                    'Training records are reviewed during internal audits per SOP-QA-010.',
                    'Training matrices are maintained showing required vs. completed training by role.',
                    'Records retention: Active employees - current plus 3 years; Terminated employees - 5 years after termination.',
                    'Annual training effectiveness review conducted by HR and Quality.',
                ],
                ['Training Record Form (HR-F-010)', 'Training Matrix Template (HR-F-011)', 'Training Effectiveness Report (HR-F-012)']
            ),
        ];
    }

    protected function sopCompetencyAssessment(): array
    {
        return [
            'name' => 'SOP-HR-003: Competency Assessment Procedure',
            'approval_state' => 'approved',
            'review_interval_days' => 365,
            'html' => $this->buildSopHtml(
                'SOP-HR-003',
                'Competency Assessment Procedure',
                'This procedure defines how employee competency is assessed and verified for quality-critical activities.',
                'Department Managers, Training Coordinators, Quality Assurance.',
                [
                    'Competency assessment is required for all personnel performing quality-affecting activities.',
                    'Assessment methods include: Written tests, practical demonstrations, supervisor observation, work product review.',
                    'Minimum passing score for written assessments: 80%.',
                    'Practical assessments require successful completion of task without critical errors.',
                    'Assessor must be qualified and independent from the person being assessed.',
                    'Failed assessments require retraining before reassessment.',
                    'Maximum of 3 assessment attempts; additional failures require management review.',
                    'Annual competency verification required for critical processes.',
                    'Competency gaps identified during assessments trigger CAPA if systemic.',
                    'Assessment records maintained per SOP-HR-002.',
                ],
                ['Competency Assessment Form (HR-F-020)', 'Written Test Template (HR-F-021)', 'Practical Assessment Checklist (HR-F-022)']
            ),
        ];
    }

    protected function sopProductionTraining(): array
    {
        return [
            'name' => 'SOP-HR-004: Production Personnel Training',
            'approval_state' => 'approved',
            'review_interval_days' => 365,
            'html' => $this->buildSopHtml(
                'SOP-HR-004',
                'Production Personnel Training Requirements',
                'This procedure defines specific training requirements for manufacturing and production personnel.',
                'Production Supervisors, Training Coordinators, Manufacturing Personnel.',
                [
                    'All production personnel must complete GMP Fundamentals training before working in production areas.',
                    'Cleanroom gowning qualification required before cleanroom entry (3 successful gowning demonstrations).',
                    'ESD (Electrostatic Discharge) awareness training required for personnel handling sensitive components.',
                    'Process-specific training required for each manufacturing operation performed.',
                    'Equipment operation training required before using any production equipment.',
                    'Safety training includes: Chemical handling, ergonomics, emergency procedures.',
                    'Quality inspection training required for personnel performing in-process checks.',
                    'DHR (Device History Record) documentation training before completing production records.',
                    'Annual refresher training on GMP, safety, and critical processes.',
                    'Cross-training documented for backup personnel on critical operations.',
                    'Training effectiveness verified through quality metrics and audit findings.',
                ],
                ['Production Training Plan (HR-F-030)', 'Gowning Qualification Record (HR-F-031)', 'Equipment Training Log (HR-F-032)']
            ),
        ];
    }

    protected function sopQualityTraining(): array
    {
        return [
            'name' => 'SOP-HR-005: Quality Personnel Training',
            'approval_state' => 'approved',
            'review_interval_days' => 365,
            'html' => $this->buildSopHtml(
                'SOP-HR-005',
                'Quality Personnel Training Requirements',
                'This procedure defines specific training requirements for Quality Assurance and Quality Control personnel.',
                'QA Manager, Quality Engineers, Quality Inspectors, Training Coordinators.',
                [
                    'All QA/QC personnel must complete comprehensive QMS training within 60 days.',
                    'ISO 13485:2016 requirements training required for all quality personnel.',
                    'FDA 21 CFR Part 820 (QSR) training required within 90 days.',
                    'Internal auditor training required before conducting audits (ISO 19011 or equivalent).',
                    'Statistical methods training for personnel performing data analysis.',
                    'Inspection techniques training including use of measurement equipment.',
                    'Nonconformance handling and CAPA process training.',
                    'Risk management training (ISO 14971) for personnel involved in risk activities.',
                    'Supplier quality training for personnel managing supplier relationships.',
                    'Regulatory update training provided when regulations change.',
                    'Lead auditor certification encouraged for senior quality personnel.',
                    'Annual refresher on regulatory requirements and internal procedures.',
                ],
                ['Quality Training Plan (HR-F-040)', 'Auditor Qualification Record (HR-F-041)', 'Regulatory Training Log (HR-F-042)']
            ),
        ];
    }
