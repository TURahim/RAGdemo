...
class SearchController extends Controller
{
    public function __construct(
        ...
        protected SearchFacetCalculator $facetCalculator,
    ) {
    }
    ...
    public function search(Request $request, SearchResultsFormatter $formatter)
    {
        ...
        $departments = $this->facetCalculator->getVisibleDepartments();

        // Calculate facets for sidebar (only if there are search results)
        $facets = $results['total'] > 0
            ? $this->facetCalculator->calculateFacets($searchOpts, $this->searchRunner)
