...
class SearchRunner
{
    ...
    /**
     * Filter entities by department (bookshelf).
     * For pages and chapters, filters by their parent book's shelf membership.
     * For books, filters by direct shelf membership.
     * Shelves themselves are excluded when this filter is applied.
     */
    protected function filterInDepartment(EloquentBuilder $query, string $input, bool $negated): void
    {
        $shelfId = intval($input);
        if ($shelfId <= 0) {
            return;
        }

        // Exclude shelves from results when filtering by department
        $query->where('entities.type', '!=', 'bookshelf');

        $filter = function (EloquentBuilder $query) use ($shelfId) {
            // For books: check direct membership in bookshelves_books
            // For pages/chapters: check their book_id's membership in bookshelves_books
            $query->whereExists(function ($subQuery) use ($shelfId) {
                $subQuery->select(DB::raw(1))
                    ->from('bookshelves_books')
                    ->where('bookshelves_books.bookshelf_id', '=', $shelfId)
                    ->where(function ($q) {
                        // For books: entities.id matches book_id in pivot
                        // For pages/chapters: entities.book_id matches book_id in pivot
                        $q->whereColumn('bookshelves_books.book_id', '=', 'entities.id')
                          ->orWhereColumn('bookshelves_books.book_id', '=', 'entities.book_id');
                    });
            });
        };
    ...
    /**
     * Filter pages by approval status.
     * Supported values: approved, in_review, draft, rejected
     * Can combine with pipe: approved|in_review
     * Only applies to pages - other entity types are excluded.
     */
    protected function filterApprovalStatus(EloquentBuilder $query, string $input, bool $negated): void
    {
        $validStatuses = ['approved', 'in_review', 'draft', 'rejected'];
        $requestedStatuses = array_filter(
            explode('|', strtolower($input)),
        ...
        // This filter only makes sense for pages
        $query->where('entities.type', '=', 'page');

        $filter = function (EloquentBuilder $query) use ($requestedStatuses) {
            $query->where(function ($q) use ($requestedStatuses) {
                foreach ($requestedStatuses as $status) {
                    $q->orWhere(function ($statusQuery) use ($status) {
                        if ($status === 'approved') {
                            // Page has an approved revision (approved_revision_id is set)
                            $statusQuery->whereNotNull('entity_page_data.approved_revision_id');
                        } else {
                            // Check the latest revision's status
                            // A page is in draft/in_review/rejected if its latest revision has that status
                            // AND it doesn't have a different approved revision
                            $statusQuery->whereExists(function ($sub) use ($status) {
                                $sub->select(DB::raw(1))
                                    ->from('page_revisions as pr')
                                    ->whereColumn('pr.page_id', '=', 'entities.id')
                                    ->where('pr.status', '=', $status)
                                    ->whereRaw('pr.id = (SELECT MAX(pr2.id) FROM page_revisions pr2 WHERE pr2.page_id = entities.id)');
                            });
                        }
                    });
                }
