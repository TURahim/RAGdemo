            ...
            <div>
                <div>
                    ...
                    <form method="get" action="{{ url('/search') }}">
                        ...
                        {{-- Quick Filters for SOP Workflow --}}
                        @if(!user()->isGuest())
                        <div class="quick-filters mb-m">
                            <h6>{{ trans('entities.search_quick_filters') }}</h6>
                            <div class="button-group">
                                <a href="{{ url('/search') }}?filters[owned_by]=me&filters[type]=page"
                                   class="button outline small @if(isset($filterMap['owned_by']) && $filterMap['owned_by'] === 'me' && isset($filterMap['type']) && $filterMap['type'] === 'page') active @endif">
                                    @icon('user') {{ trans('entities.search_my_sops') }}
                                </a>
                                @if(userCan('content-export'))
                                <a href="{{ url('/search') }}?filters[approval_status]=in_review&filters[type]=page"
                                   class="button outline small @if(isset($filterMap['approval_status']) && $filterMap['approval_status'] === 'in_review') active @endif">
                                    @icon('check-circle') {{ trans('entities.search_pending_review') }}
                                </a>
                                @endif
                            </div>
                        </div>
                        ...
                        @if(isset($departments) && $departments->count() > 0)
                            <h6>{{ trans('entities.search_department_filter') }}</h6>
                            @include('search.parts.department-filter', ['departments' => $departments, 'filters' => $filterMap])
                        @endif

                        {{-- Approval Status Filter --}}
                        @include('search.parts.status-filter', ['filters' => $filterMap])
                    ...
                    {{-- Faceted Results Sidebar --}}
                    @if(isset($facets))
                        @include('search.parts.facets', ['facets' => $facets, 'options' => $options])
                    @endif
                </div>
            </div>
            <div>
                <div class="card content-wrap">
                    ...
                    @if(!empty($filterMap))
                    <div class="active-filters mb-m">
                        @if(isset($filterMap['in_department']))
                            @php
                                $deptName = $departments->firstWhere('id', $filterMap['in_department'])?->name ?? trans('entities.search_unknown_department');
                            @endphp
                            <span class="filter-tag">
                                {{ trans('entities.search_in_department') }}: {{ $deptName }}
                                <a href="{{ url('/search') }}?{{ http_build_query(array_merge(request()->except(['filters.in_department', 'page']), [])) }}"
                                   class="filter-remove" title="{{ trans('common.remove') }}">@icon('close')</a>
                            </span>
                        @endif
                        @if(isset($filterMap['approval_status']))
                            <span class="filter-tag">
                                {{ trans('entities.search_approval_status') }}: {{ trans('entities.search_status_' . $filterMap['approval_status']) }}
                                <a href="{{ url('/search') }}?{{ http_build_query(request()->except(['filters.approval_status', 'page'])) }}"
                                   class="filter-remove" title="{{ trans('common.remove') }}">@icon('close')</a>
                            </span>
                        @endif
                    </div>
